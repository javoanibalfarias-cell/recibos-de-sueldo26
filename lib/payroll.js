...
function readRemuneraciones(ws, r){
  ...
  if(sueldoBasico > 0) add('I','Ausentismo', Math.round(sueldoBasico * 0.15), '15%');
  add('K','Días trabajados / ajuste', rowCell(ws,'K',r),'J');
  add('M','Presentismo', rowCell(ws,'M',r),'L');
  // Antigüedad: 10% del sueldo básico
  if(sueldoBasico > 0) add('O','Antigüedad', Math.round(sueldoBasico * 0.10), '10%');

  add('Q','Adicional remunerativo', rowCell(ws,'Q',r),'P');
  // Adicional fijo / porcentaje: 10% del sueldo básico
  if(sueldoBasico > 0) add('S','Adicional fijo / porcentaje', Math.round(sueldoBasico * 0.10), '10%');
...
function readRetenciones(ws, r){
  ...
  if(bruto > 0) addD('E','Jubilación', Math.round(bruto * 0.11), '11%');
  // Ley 19032 / INSSJP: 3% del bruto
  if(bruto > 0) addD('G','Ley 19032 / INSSJP', Math.round(bruto * 0.03), '3%');
  // Obra social media jornada: 3% del bruto
  if(bruto > 0) addD('I','Obra social media jornada', Math.round(bruto * 0.03), '3%');
