...
function fmt(n){ try { const rounded = Math.round(Number(n||0)); return new Intl.NumberFormat('es-AR',{style:'currency',currency:'ARS',minimumFractionDigits:0,maximumFractionDigits:0}).format(rounded); } catch { return String(n||0); } }
function num(v){ if(v==null||v===''||String(v).startsWith('#')) return 0; if(typeof v==='number') return v; const s=String(v).replace(/\./g,'').replace(',','.').replace(/[^0-9.-]/g,''); const x=parseFloat(s); return isNaN(x)?0:Math.round(x); }
...
function loadPayrollFromExcel(excelPath, periodo){
  ...
  for(let r=8; r<=last; r++){
    ...
    const bruto = Math.round(remuneracion.remunerativos.reduce((a,c)=>a+c.importe,0));
    const noRem = Math.round(remuneracion.noRemunerativos.reduce((a,c)=>a+c.importe,0) + retencion.noRemunerativos.reduce((a,c)=>a+c.importe,0));
    const desc = Math.round(retencion.descuentos.reduce((a,c)=>a+c.importe,0));
...
function readRemuneraciones(ws, r){
  const conceptos = [];
  const add = (col, label, importe, qtyCol, pct) => { const amount = num(importe); if(amount>0) conceptos.push({codigo:col, concepto:label, cantidad: qtyCol ? text(rowCell(ws, qtyCol, r)) : (pct||''), importe: Math.round(amount)}); };
  const sueldoBasico = num(rowCell(ws,'E',r));
  add('E','Sueldo básico / sueldo exacto', rowCell(ws,'E',r));
  add('G','Básico días normales', rowCell(ws,'G',r),'F');
  // Ausentismo: 15% del sueldo básico
  if(sueldoBasico > 0) add('I','Ausentismo', Math.round(sueldoBasico * 15 / 100), '15%');
  add('K','Días trabajados / ajuste', rowCell(ws,'K',r),'J');
  add('M','Presentismo', rowCell(ws,'M',r),'L');
  // Antigüedad: 10% del sueldo básico
  if(sueldoBasico > 0) add('O','Antigüedad', Math.round(sueldoBasico * 10 / 100), '10%');

  add('Q','Adicional remunerativo', rowCell(ws,'Q',r),'P');
  // Adicional fijo / porcentaje: 10% del sueldo básico
  if(sueldoBasico > 0) add('S','Adicional fijo / porcentaje', Math.round(sueldoBasico * 10 / 100), '10%');
  add('U','Falla de caja', rowCell(ws,'U',r),'T'); add('W','Adicional remunerativo 2',rowCell(ws,'W',r),'V'); add('Y','Adicional remunerativo 3',rowCell(ws,'Y',r),'X');
...
function readRetenciones(ws, r){
  ...
  const addD = (col, label, amount, pctCol, pct) => { const n = num(amount); if(n>0) descuentos.push({codigo:col, concepto:label, porcentaje:pctCol?text(rowCell(ws,pctCol,r)):(pct||''), importe:Math.round(n)}); };
  const addN = (col, label, amount) => { const n = num(amount); if(n>0) noRem.push({codigo:col, concepto:label, importe:Math.round(n)}); };
  
  // Obtener el bruto total desde fila anterior
  const brutoRow = num(rowCell(ws,'AP',r-1)) || 0;
  const bruto = brutoRow > 0 ? brutoRow : num(rowCell(ws,'AP',r));
  
  // Jubilación: 11% del bruto
  if(bruto > 0) addD('E','Jubilación', Math.round(bruto * 11 / 100), '11%');
  // Ley 19032 / INSSJP: 3% del bruto
  if(bruto > 0) addD('G','Ley 19032 / INSSJP', Math.round(bruto * 3 / 100), '3%');
  // Obra social media jornada: 3% del bruto
  if(bruto > 0) addD('I','Obra social media jornada', Math.round(bruto * 3 / 100), '3%');
  
  addD('M','Retención 2', rowCell(ws,'M',r),'L'); addD('O','Retención 3', rowCell(ws,'O',r),'N');
  ...
  addD('Z','Anticipos', rowCell(ws,'Z',r)); addD('AA','Anticipo SAC', rowCell(ws,'AA',r)); addD('AB','Obra Social Acuerdos', rowCell(ws,'AB',r));
