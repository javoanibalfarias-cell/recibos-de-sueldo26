...
function readRemuneraciones(ws, r){
  ...
  const formatQty = value => {
    ...
    return /^[A-Z]+$/.test(s) ? text(rowCell(ws, s, r)) : s;
  };
  const add = (col, label, importe, qty) => {
    const amount = num(importe);
    if(amount > 0) conceptos.push({codigo: col, concepto: label, cantidad: formatQty(qty), importe: Math.round(amount)});
  };
...
function readRetenciones(ws, r){
  ...
  const addD = (col, label, amount, pct) => { const n = num(amount); if(n>0) descuentos.push({codigo:col, concepto:label, porcentaje: formatPct(pct), importe:Math.round(n)}); };
