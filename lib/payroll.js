...
function readRemuneraciones(ws, r){
  const conceptos = [];
  const add = (col, label, importe, qtyCol, pct) => { const amount = num(importe); if(amount>0) conceptos.push({codigo:col, concepto:label, cantidad: qtyCol ? text(rowCell(ws, qtyCol, r)) : (pct||''), importe: amount}); };
...
function readRetenciones(ws, r){
  ...
  const addD = (col, label, amount, pctCol, pct) => { const n = num(amount); if(n>0) descuentos.push({codigo:col, concepto:label, porcentaje:pctCol?text(rowCell(ws,pctCol,r)):(pct||''), importe:n}); };
  ...
  if(bruto > 0) addD('E','Jubilación', bruto * 0.11, '11%');
  // Ley 19032 / INSSJP: 3% del bruto
  if(bruto > 0) addD('G','Ley 19032 / INSSJP', bruto * 0.03, '3%');
  // Obra social media jornada: 3% del bruto
  if(bruto > 0) addD('I','Obra social media jornada', bruto * 0.03, '3%');
